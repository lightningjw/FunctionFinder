...
final class DatabaseManager {
    ...
    public func createPost(newPost: Post, completion: @escaping (Bool) -> Void) {
        ...
        guard let data = newPost.asDictionary()
        else {
            completion(false)
            return
        }
    ...
    public func createUser(newUser: User, completion: @escaping (Bool) -> Void) {
        ...
        guard let data = newUser.asDictionary()
        else {
            completion(false)
            return
        }
    ...
    public func getUserCounts(username: String, completion: @escaping ((followers: Int, following: Int, posts: Int)) -> Void) {
        ...
        userRef.collection("posts").getDocuments { snapshot, error in
            ...
            guard let count = snapshot?.documents.count, error == nil
            else {
                return
            }
            posts = count
    ...
    public func isFollowing(targetUsername: String, completion: @escaping (Bool) -> Void) {
        ...
        let ref = database.collection("users")
            ...
            .document(currentUsername)
        ref.getDocument { snapshot, error in
            guard snapshot?.data() != nil, error == nil
            else {
                // Not following
                completion(false)
                return
    ...
    /// Gets users that the username is following
    ...
    public func createComment(comment: Comment, postID: String, owner: String, completion: @escaping (Bool) -> Void) {
        ...
        completion: @escaping (Bool) -> Void)
    {
        ...
        getPost(with: postID, from: owner) { post in
            ...
            guard let data = post.asDictionary()
            else {
                completion(false)
                return
            }
